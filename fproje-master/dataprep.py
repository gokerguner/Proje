...
def selected_dataset(labels,label_map,dir_,train_percent,val_percent,test_percent):
    sampleset = pd.read_csv('loader.csv')
    selected_samples = sampleset[sampleset['annotation_value'].isin(labels)]
    selected_samples.to_csv('selecteds.csv',index=None)
    ...
    transform = transforms.Compose([ds.RandomCrop(224), ds.RandomRotate(),ds.ToTensor()])
    image_datasets = {x[0]: ds.ImageDataset(x[1],dir_, label_map, transform=transform)
                        for x in [['train','trains.csv'],['val','vals.csv'],['test','tests.csv']]}
