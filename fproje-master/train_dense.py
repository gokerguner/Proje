    ...
    for epoch in range(num_epochs):
		...
		scheduler.step(running_loss)
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
		torch.save(best_model_wts, '/mnt/data/data/summer_2018/model_dense_modified.pth')
...
def model_training(model_ft, device, dataloaders, dataset_sizes):
    ...
    criterion = nn.CrossEntropyLoss(weight = torch.cuda.FloatTensor([0.02, 0.037, 0.14, 0.24, 0.42]))
